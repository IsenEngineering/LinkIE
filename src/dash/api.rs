...
use axum::{Json, Router, extract::State, routing::{delete, get, patch, post}};
use reqwest::StatusCode;
use serde::Deserialize;
use crate::{SharedCollection, collection::Link};
...
) -> Json<HashMap<String, Link>> {
    Json(collection.read().await.list())
}
...
async fn create_link(
    ...
    match (payload.subdomain, payload.path) {
        (Some(subdomain), Some(path)) => {
            collection.new_subdomain_with_path(subdomain, path, payload.destination);
        },
        (None, Some(path)) => {
            collection.new_path(path, payload.destination);
        },
        (Some(subdomain), None) => {
            collection.new_subdomain(subdomain, payload.destination);
        },
        (None, None) => {
...
async fn update_link(
    ...
    if let Some(dest) = payload.new_destination &&
        let Err(()) = collection.update_destination(&key, dest) {
        // en théorie cette condition ne sera jamais remplie
        return Err((StatusCode::NOT_FOUND, "Cette redirection n'existe pas"))
    }

    if payload.new_path.is_some() || payload.new_subdomain.is_some() {
        collection.update_key(&key, (payload.new_subdomain, payload.new_path)).unwrap();
    }
...
async fn delete_link(
    ...
    match collection.write().await.remove(key) {
        Ok(()) => Ok("Supprimé"),
        Err(()) => Err((StatusCode::NOT_FOUND, "Cette redirection n'existe pas"))
    }
}

pub fn endpoints() -> Router<SharedCollection> {
    Router::new()
